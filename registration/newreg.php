...
if ( isset($_POST['submit']) ) {
	...
	if ( $selectSQL ) {
		...
		$errors['database'] = '<span class="text-red">Something went wrong, please contact to support team!</span>';
	}

	if ( empty($_POST['first_name']) ) {
		$errors['first_name'] = '<span class="text-red">Please enter your first name!</span>';
	}
	if ( empty($_POST['middle_name']) ) {
		$errors['middle_name'] = '<span class="text-red">Please enter your middle name!</span>';
	}
	if ( empty($_POST['last_name']) ) {
		$errors['last_name'] = '<span class="text-red">Please enter your last name!</span>';
	}
	if ( empty($_POST['dob']) ) {
		$errors['dob'] = '<span class="text-red">Please enter your date of birth!</span>';
	}
	if ( empty($_POST['gender']) ) {
		$errors['gender'] = '<span class="text-red">Please select your gender!</span>';
	}
	if ( empty($_POST['merital_status']) ) {
		$errors['merital_status'] = '<span class="text-red">Please choose your merital status!</span>';
	}
	if ( empty($_POST['nationality']) ) {
		$errors['nationality'] = '<span class="text-red">Please enter your nationality!</span>';
	}
	if ( empty($_POST['address']) ) {
		$errors['address'] = '<span class="text-red">Please enter your address!</span>';
	}
	if ( empty($_POST['city']) ) {
		$errors['city'] = '<span class="text-red">Please enter your city!</span>';
	}
	if ( empty($_POST['state']) ) {
		$errors['state'] = '<span class="text-red">Please enter your state!</span>';
	}
	if ( empty($_POST['country']) ) {
		$errors['country'] = '<span class="text-red">Please enter your country!</span>';
	}
	if ( empty($_POST['email']) ) {
		$errors['email'] = '<span class="text-red">Please enter your email address!</span>';
	}
	if ( empty($_POST['mobile']) ) {
		$errors['mobile'] = '<span class="text-red">Please enter your mobile number!</span>';
	}
	if ( empty($_POST['identification']) ) {
		$errors['identification'] = '<span class="text-red">Please choose your identification document!</span>';
	}
	if ( empty($_POST['id_no']) ) {
		$errors['id_no'] = '<span class="text-red">Please enter your identification number!</span>';
	}
	if ( empty($_POST['employment_type']) ) {
		$errors['employment_type'] = '<span class="text-red">Please choose your employment type!</span>';
	}
	...
	if ( empty($_FILES['photo']['name']) ) {
		$errors['photo'] = '<span class="text-red">Please upload your recent photograph!</span>';
	} else {
		...
		if ( in_array($file_ext, $expensions) === false ) {
		 	$errors['photo'] = '<span class="text-red">Extension not allowed, please choose a JPEG or PNG file!</span>';
		}
	}
...
<body class="hold-transition register-page" style="background:#00162A">
	<div class="container">
		<div class="register-box">
		  	<div class="register-logo" style="color: white">
		    	<a href="<?php echo BASE_URL; ?>" style="color: white"><b>Online Payroll</b> Management</a>
		    	<small>Employee Registration Form</small>
		  	</div>
		</div>
		<div class="box box-primary" style="background:#464646">
			<div class="box-header with-border" style="color: white">
				<h3 class="box-title">Fill the below form</h3>
				<div class="box-tools pull-right">
					<span class="text-white">All fields are mandatory</span>
				</div>
			</div>
			<form class="form-horizontal" method="post" enctype="multipart/form-data" novalidate="" style="color: white">
				<div class="box-body">
					<div class="form-group">
						...
						<div class="col-sm-5" style="width: 250px;">
							<input type="text" class="form-control" id="middle_name" name="middle_name"placeholder="Middle Name" value="<?php echo $_POST['middle_name']; ?>" required />
							<?php echo $errors['middle_name']; ?>
						</div>
						<div class="col-sm-5" style="width: 250px;">
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="number" class="form-control" id="mobile" name="mobile" placeholder="+63" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength = "10"value="<?php echo $_POST['mobile']; ?>" required />
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<select class="form-control" id="identification" name="identification" required>
								...
								<option <?php echo $_POST['identification']=='GSIS ID or GSIS UMID Card' ? 'selected' : ''; ?> value="GSIS ID or GSIS UMID Card">GSIS ID or GSIS UMID Card</option>
								<option <?php echo $_POST['identification']=='SSS ID or SSS UMID Card' ? 'selected' : ''; ?> value="SSS ID or SSS UMID Card">SSS ID or SSS UMID Card</option>
								<option <?php echo $_POST['identification']=='NBI Clearance' ? 'selected' : ''; ?> value="NBI Clearance">NBI Clearance</option>
								<option <?php echo $_POST['identification']=='PhilHealth ID' ? 'selected' : ''; ?> value="PhilHealth ID">PhilHealth ID</option>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<select class="form-control" id="employment_type" name="employment_type">
								...
								<option <?php echo $_POST['employment_type']=='Full-Time' ? 'selected' : ''; ?> value="Full-Time">Full-Time</option>
								<option <?php echo $_POST['employment_type']=='Contract Worker' ? 'selected' : ''; ?> value="Contract Worker">Contract Worker</option>
							</select>
				...
				<div class="box-footer" style="background:#464646">
