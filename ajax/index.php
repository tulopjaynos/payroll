...
function LoadingEmployees() {
	...
	if ( !empty($requestData['search']['value']) ) {
		...
		$sql .= " OR `emp_password` LIKE '" . $requestData['search']['value'] . "%'";
	...
	while ( $row = mysqli_fetch_assoc($query) ) {
		...
		$nestedData[] = strtoupper($row["emp_password"]);
